...
def predecir_cliente(cliente: dict) -> tuple[int | None, str | None, np.ndarray, float, int]:
    datos = cliente.copy()
    cliente_id = datos.pop("cliente_id", None)
    nombre = datos.pop("nombre", None)

    cliente_limpio = {
        k: float(v) if hasattr(v, "__float__") else v for k, v in datos.items()
    }
    ...
    return cliente_id, nombre, datos_procesados, prob_abandono, prediccion
